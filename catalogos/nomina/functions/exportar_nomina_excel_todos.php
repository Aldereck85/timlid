...
$stmt = $conn->prepare("SELECT ne.*,n.*, np.tipo_id, DATE_FORMAT(n.fecha_inicio, '%d/%m/%Y') as fechaInicio, DATE_FORMAT(n.fecha_fin, '%d/%m/%Y') as fechaFin, DATE_FORMAT(n.fecha_pago, '%d/%m/%Y') as fechaPago, e.PKEmpleado, e.Nombres, e.PrimerApellido, e.SegundoApellido,dle.FechaIngreso,dle.Sueldo,pp.DiasPago, s.sucursal, s.id as sucursal_id, pp.Periodo, pp.PKPeriodo_pago 
    FROM nomina as n 
      INNER JOIN nomina_empleado as ne ON n.id = ne.FKNomina
      LEFT JOIN nomina_principal as np ON np.id = n.fk_nomina_principal
      ...
      LEFT JOIN sucursales as s ON s.id = np.sucursal_id WHERE n.id = :idNomina ");
...
$stmt = $conn->prepare('SELECT e.PKEmpleado,e.Nombres, e.PrimerApellido, e.SegundoApellido, t.Turno, dme.NSS, p.puesto, e.RFC,  DATE_FORMAT(dle.FechaIngreso, "%d/%m/%Y") as FechaIngresoFormat, ne.* FROM empleados as e INNER JOIN nomina_empleado as ne ON ne.FKNomina = :idNomina AND ne.FKEmpleado = e.PKEmpleado INNER JOIN datos_laborales_empleado as dle ON dle.FKEmpleado = e.PKEmpleado INNER JOIN periodo_pago as pp ON pp.PKPeriodo_pago = dle.FKPeriodo LEFT JOIN datos_medicos_empleado as dme ON dme.FKEmpleado = e.PKEmpleado LEFT JOIN puestos as p ON p.id = dle.FKPuesto LEFT JOIN turnos as t ON t.PKTurno = dle.FKTurno LEFT JOIN sucursales as s ON s.id = dle.FKSucursal WHERE e.estatus = 1 AND e.is_generic = 0 AND dle.FKSucursal = :idsucursal AND dle.FKPeriodo = :idperiodo AND e.empresa_id = '.$_SESSION['IDEmpresa']);
$stmt->bindValue(":idNomina", $idNomina);
$stmt->bindValue(":idsucursal", $datosNomina['sucursal_id']);
$stmt->bindValue(":idperiodo", $datosNomina['PKPeriodo_pago']);
$stmt->execute();
$datosEmpleados = $stmt->fetchAll();

$index = 0;
$cuenta_empleado = count($datosEmpleados);
...
foreach($datosEmpleados as $de){
  ...
  $idNominaEmpleado = $de['PKNomina'];
  $nombreEmpleadoActual = $de['Nombres']." ".$de['PrimerApellido'].' '.$de['SegundoApellido'];
  $tituloNombre = substr($nombreEmpleadoActual, 0, 31);
  $spreadsheet->getActiveSheet()->setTitle($tituloNombre);
  ...
  $sheet->setCellValue('B3', $nombreEmpleadoActual);
  ...
  $sheet->setCellValue('B'.$fila, $de['Turno']); 

  $spreadsheet->getActiveSheet()->getStyle('D'.$fila)->applyFromArray($styleTitle);
  $sheet->setCellValue('D'.$fila, "NSS: "); 
  $sheet->setCellValue('E'.$fila, $de['NSS']); 

  $spreadsheet->getActiveSheet()->getStyle('G'.$fila)->applyFromArray($styleTitle);
  $sheet->setCellValue('G'.$fila, "Puesto: "); 
  $sheet->setCellValue('H'.$fila, $de['puesto']); 
  $fila++;

  $spreadsheet->getActiveSheet()->getStyle('A'.$fila)->applyFromArray($styleTitle);
  $sheet->setCellValue('A'.$fila, "RFC: "); 
  $sheet->setCellValue('B'.$fila, $de['RFC']); 

  $spreadsheet->getActiveSheet()->getStyle('D'.$fila)->applyFromArray($styleTitle);
  $sheet->setCellValue('D'.$fila, "Fecha de ingreso: "); 
  $sheet->setCellValue('E'.$fila, $de['FechaIngresoFormat']); 
  $fila++;
  ...
  foreach($datosPercepciones as $dp){
    ...
    $sheet->setCellValue('A'.$fila_perc, $dp['clave']." - ".$dp['codigo']." - ".$dp['concepto']);
  ...
  if($de['SAE'] > 0){
    ...
    $sheet->setCellValue('D'.$fila_perc, number_format($de['SAE'],2));
    $fila_perc++;

    $total_percepciones = $total_percepciones + $de['SAE'];
  }

  //OTROS PAGOS
  $stmt = $conn->prepare("SELECT * FROM detalle_otros_pagos_nomina_empleado as dnope INNER JOIN otros_pagos as op ON op.id = dnope.otros_pagos_id 
  INNER JOIN relacion_concepto_otros_pagos as rcop ON dnope.relacion_concepto_otros_pagos_id = rcop.id AND rcop.empresa_id = :empresa_id WHERE dnope.nomina_empleado_id = :idNominaEmpleado ");
  $stmt->bindValue(':empresa_id',$_SESSION['IDEmpresa']);
  $stmt->bindValue(':idNominaEmpleado',$idNominaEmpleado);
  $stmt->execute();
  ...
  if($de['ISR'] > 0){
    ...
    $sheet->setCellValue('N'.$fila_ded, number_format($de['ISR'],2));
    $sheet->setCellValue('O'.$fila_ded, 0.00);
    $sheet->setCellValue('P'.$fila_ded, number_format($de['ISR'],2));
    $fila_ded++;

    $total_deducciones = $total_deducciones + $de['ISR'];
  }
  ...
  if($de['cuotaIMSS'] > 0){
    ...
    $sheet->setCellValue('N'.$fila_ded, number_format($de['cuotaIMSS'],2));
    $sheet->setCellValue('O'.$fila_ded, 0.00);
    $sheet->setCellValue('P'.$fila_ded, number_format($de['cuotaIMSS'],2));
    $fila_ded++;

    $total_deducciones = $total_deducciones + $de['cuotaIMSS'];
  }
  ...
  if($index < $cuenta_empleado){
    $spreadsheet->createSheet();
  }

}

$archivo = "nomina_".$datosNomina['no_nomina'];
