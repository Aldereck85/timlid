    ...
    if($tipo == 7){
      ...
      if($idCuentaMovActual == $cuentaDestAnterior && $tipoAnterior == 3){ //el movimiento anterior es una transferencia al mov actual buscado
        ...
        if($referenciaA != "" || $referenciaA != "-"){
          ...
          $stmt1 = $conn->prepare('SELECT * FROM  cuenta_caja_chica WHERE FKCuenta = :fkcuenta');
          $stmt1->bindValue(':fkcuenta',$idCuenta,PDO::PARAM_INT);
          $stmt1->execute();
          $rowc = $stmt1->fetch();
          $saldoI = $rowc['SaldoInicialCaja'];

          //actualiza saldo de la cuenta actual
          $saldoFinal=$saldoI-$deposito;
          $stmtU = $conn->prepare('UPDATE cuenta_caja_chica SET SaldoInicialCaja =:saldoFinal WHERE FKCuenta = :fkcuenta');
          ...
          $stmtD = $conn->prepare('SELECT * FROM  cuenta_caja_chica WHERE FKCuenta = :fkcuenta');
          $stmtD->bindValue(':fkcuenta',$cuentaDestAnterior, PDO::PARAM_INT);
          $stmtD->execute();
          $rowDes = $stmtD->fetch();
          $saldoFAnt = $rowDes['SaldoInicialCaja'];
      ...
      if($tipo == 3){ //Busca el siguiente movimiento
        ...
        if($idDestino == $cuentaSiguiente && $tipoSiguiente == 7){ //el movimiento anterior es una transferencia al mov actual buscado
          ...
          $stmtD->execute();
          $rowDes = $stmtD->fetch();
          $saldoFSig = $rowDes['SaldoInicialCaja'];
          //actualiza el saldo de la cunata anterior
        ...
        if($tipo == 6 || $tipo == 2){
          ...
          $stmtD->execute();
          $rowDes = $stmtD->fetch();
          $saldoAc = $rowDes['SaldoInicialCaja'];
          //actualiza el saldo de la cunata anterior
