...
$stmt = $conn->prepare('call spc_Salidas()');
$stmt->execute();
$table="";
...
while (($row = $stmt->fetch()) !== false) {
    ...
    $table.='{"id":"<label class=\"textTable\">'.$row['PKHistorialInsumos'].'</label>",
        ...
        "Cantidad saliente":"<label class=\"textTable\">'.$row['CantidadMovimiento'].'</label>",
        "Descripcion":"<label class=\"textTable\">'.$row['Descripcion'].'</label>",
        "Fecha de salida":"<label class=\"textTable\">'.$row['FechaEdicion'].'</label>",
