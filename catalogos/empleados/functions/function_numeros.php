...
class NumberToLetterConverter {
  ...
  private $MONEDAS = array(
    ...
    public function numero_completo($numero) {

      $numerocompleto = explode('.', $numero);
      $decimal = $this->to_word($numerocompleto[0]);
      $fracion = $numerocompleto[1].'/100';
      
      if($numerocompleto[0]== 0){
        $moneda = "CERO PESOS ";
      }
      if($numerocompleto[0]== 1){
        $moneda = " PESO ";
      }
      if($numerocompleto[0] > 1){
        $moneda = " PESOS ";
      ...
      $cadena_numero = $decimal.$moneda.$fracion;
    ...
    private function to_word($number, $miMoneda = null) {
