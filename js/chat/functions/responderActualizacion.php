...
$idchatpadre = $_POST['IDChatPadre'];

$json = new \stdClass();
try {
    $stmt = $conn->prepare('INSERT INTO chat (Contenido, FKUsuario, FechaAlta, FKTarea, Tipo, ChatPadre, Anclar) VALUES(:content, :idusuario, NOW(), :fktarea, 2, :chatpadre, 0)');
    $stmt->bindValue(':content', $guardar);
    $stmt->bindValue(':idusuario', $idusuario);
    $stmt->bindValue(':fktarea', $idtarea);
    $stmt->bindValue(':chatpadre', $idchatpadre);
    if ($stmt->execute()) {
        ...
        if ($count > 0) {
            foreach ($tareas as $tarea) {
                if ($tarea['FKUsuario'] > 0) {
                    ...
                    $stmt->execute([':tipoNot' => 2, ':detaleNot' => 23, ':idElem' => $tarea['FKProyecto'], ':idSubElem' => $idtarea, ':fecha' => $timestamp, ':usrCreo' => $_SESSION['PKUsuario'], ':usrRecibe' => $tarea['FKUsuario']]);
                }
            }
        }
