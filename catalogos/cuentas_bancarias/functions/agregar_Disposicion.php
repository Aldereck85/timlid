    ...
    try {
        if ($hayCuenta == 1) { //IF exist account
            ...
            if ($tipoC == 2) {
                ...
                $saldoR = $saldoUtulizadoDest - $cantidadDisponer;
                $stmt = $conn->prepare('UPDATE cuentas_credito SET Credito_Utilizado =:saldoFinal WHERE FKCuenta =:fkcuenta');
                $stmt->bindValue(':fkcuenta', $idACuenta, PDO::PARAM_INT);
                ...
                $stmt = $conn->prepare('INSERT INTO movimientos_cuentas_bancarias_empresa (cuenta_origen_id,Fecha,tipo_movimiento_id,Descripcion,Retiro,Comprobado,Saldo,cuenta_destino_id) VALUES (:fkcuenta,:fecha,:tipo,:descripcion,:retiro,:comprobado,:saldo,:iddestino)');
                $stmt->bindValue(':fkcuenta', $id, PDO::PARAM_INT);
                ...
                $saldoFOtras = $SaldoInicial + $cantidadDisponer;
            ...
            //REGISTRO DEL MOVIMIENTO 1
            $stmt = $conn->prepare('INSERT INTO movimientos_cuentas_bancarias_empresa (cuenta_origen_id,Fecha,tipo_movimiento_id,Descripcion,Retiro,Comprobado,saldo) VALUES (:fkcuenta,:fecha,:tipo,:descripcion,:retiro,:comprobado,:saldo)');
            $stmt->bindValue(':fkcuenta', $id, PDO::PARAM_INT);
            //$stmt->bindValue(':responsable',$responsable);
            $stmt->bindValue(':fecha', $fecha);
            $stmt->bindValue(':tipo', 4);
