...
$stmtp = $conn->prepare("SELECT distinct p.Nombre as nombre,
                          ...
                          ifnull(epp.numero_serie,'') as noSerie,
                          if( epp.caducidad = null or epp.caducidad = '0000-00-00','', epp.caducidad) as caducidad
                          from inventario_entrada_por_sucursales ieps
                            inner join sucursales s on s.id = ieps.sucursal_origen_id 
                            ...
                            inner join existencia_por_productos epp on p.PKProducto = epp.producto_id and ieps.sucursal_origen_id = epp.sucursal_id and ifnull(ieps.numero_lote,'') = ifnull(epp.numero_lote,'') and ifnull(ieps.numero_serie,'') = ifnull(epp.numero_serie,'')
