...
$pdfdoc = $pdf->Output("Requisición de Compra". $GLOBALS["folio"], "S");

// send message
try {    
    $mail->Sender = $email_origen;
    $mail->setFrom($email_origen, $GLOBALS["NombreCliente"]);
    $mail->addAddress($GLOBALS["EmailComprador"]); 

    //Attachments
    $mail->AddStringAttachment($pdfdoc, 'Requisicion_de_Compra_'.$GLOBALS["folio"].'.pdf');

    $mensaje = "¡Tienes una nueva requisición que atender! ";
    //Content
    $mail->isHTML(true);                                  //Set email format to HTML
    $mail->Subject = utf8_decode('Requisición de compra #'.$GLOBALS["folio"]);
    $mail->Body    = utf8_decode($mensaje.'<br><br> <a href="https://app.timlid.com/catalogos/inventarios_productos/catalogos/requisicion_compra/detalleRequisicion.php?requisicion='.$GLOBALS['PKRequi'].'" style="background-color: #006dd9; /* TIMLID */
    border: none;
    color: white;
    padding: 15px 32px;
    text-align: center;
    ...
    display: inline-block;
    font-size: 16px;">Ver Requisición de Compra</a>');
