...
$stmt = $conn->prepare("SELECT e.Nombres, e.PrimerApellido, e.SegundoApellido, e.CURP, e.CP, dme.NSS as num_seguridad_social, e.RFC, e.idtimbrado, e.email, CONCAT(dle.FechaIngreso,'T06:00:00.000Z') as fecha_inicio_rel_laboral, dle.FechaIngreso, tc.codigo as tipo_contrato, tj.codigo as tipo_jornada, tr.codigo as tipo_regimen, e.id_empleado as num_empleado, p.puesto, rp.codigo as riesgo_puesto,  pp.Codigo as periodicidad_pago, ef.Codigo as clave_ent_fed, dle.Sueldo, crf.clave as clave_regimen_fiscal, dle.SalarioBaseCotizacionFijo, dle.SalarioBaseCotizacionVariable FROM empleados as e LEFT JOIN datos_medicos_empleado as dme ON dme.FKEmpleado = e.PKEmpleado LEFT JOIN datos_laborales_empleado as dle ON dle.FKEmpleado = e.PKEmpleado LEFT JOIN tipo_contrato as tc ON tc.id = dle.FKTipoContrato LEFT JOIN turnos as t ON t.PKTurno = dle.FKTurno LEFT JOIN tipo_jornada as tj ON tj.id = t.tipo_jornada_id LEFT JOIN tipo_regimen as tr ON tr.id = dle.FKRegimen LEFT JOIN puestos as p ON p.id = dle.FKPuesto LEFT JOIN riesgo_puesto as rp ON rp.id = dle.FKRiesgoPuesto LEFT JOIN periodo_pago as pp ON pp.PKPeriodo_pago = dle.FKPeriodo LEFT JOIN estados_federativos as ef ON ef.PKEstado = e.FKEstado LEFT JOIN claves_regimen_fiscal as crf ON crf.id = e.claves_regimen_fiscal_id WHERE e.PKEmpleado = :idEmpleado AND e.empresa_id = :empresa_id");
...
if(trim($datos_empleado['CP']) == ""){
    $respuesta->estatus = "fallo-general";
    $respuesta->mensaje = "Ingresa el código postal del empleado para poder timbrar.";
    $respuesta = json_encode($respuesta);
    echo $respuesta;
    return;
}
...
$antiguedad = calcularDiferencia($datos_empleado['FechaIngreso']." 06:00:00", $datos_nomina['fecha_fin']." 06:00:00");
...
/*echo "<pre>",print_r($conceptos),"</pre>";
return;*/
$x = 0;
...
/*echo "<pre>",print_r($percepcion),"</pre>";
return;*/
