        ...
        if ($stmt->execute()) {
            try {
                ...
                if ($stmtI->execute($datos)) {
                    ...
                    $stmt = $conn->prepare('SELECT cc.SaldoInicialCaja as saldoAnterior, mcb.PKMovimiento FROM cuenta_caja_chica as cc INNER JOIN movimientos_cuentas_bancarias_empresa as mcb ON cc.FKCuenta = mcb.cuenta_origen_id WHERE mcb.PKMovimiento = :idLast');
