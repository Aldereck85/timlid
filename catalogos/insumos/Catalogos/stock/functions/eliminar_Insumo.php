...
session_start();
    ...
    if(isset($_SESSION["Usuario"])){
      ...
      try{
        $stmt = $conn->prepare('call spd_EliminarInsumoUnico(:id,:usuario)');
        $stmt->bindValue(':id',$id,PDO::PARAM_INT);
        $stmt->bindValue(':usuario',$usuario,PDO::PARAM_STR);

        /*$stmt->bindValue(':id', $id, PDO::PARAM_INT);*/
        if($stmt->execute()){
          $_SESSION['message'] = 'Operación exitosa. Insumo eliminado correctamente.';
          $_SESSION['message_type'] = 'success';

          header("Location: ../");
        }else{
          echo "fallo";
          $_SESSION['message'] = 'Operación fallida. Insumo no eliminado correctamente.';
