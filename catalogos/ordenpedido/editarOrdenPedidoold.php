...
if (isset($_SESSION["Usuario"])) {
    ...
    if (isset($_POST['idCotizacionU'])) {
        ...
        $stmt = $conn->prepare('SELECT c.PKCotizacion, c.id_cotizacion_empresa,c.ImporteTotal, c.Subtotal, c.FechaIngreso, 
          c.FechaVencimiento, c.FKCliente, c.NotaCliente,c.NotaInterna, cl.razon_social, c.FKSucursal, c.estatus_factura_id
        ...
        $estatus = $row['estatus_factura_id'];
    } else {
       // header("location:/.");
    }
} else {
                          ...
                          <div class="row">
                            <div class="col-lg-6">
                              ...
                              <input type="text" class="form-control alphaNumeric-only" name="txtReferencia"
                                id="txtReferencia" maxlength="10" value="<?=sprintf("%011d",$ReferenciaPorEmpresa)?>" readonly>
                            </div>
                            <div class="col-lg-6">
                                ...
                                <option value="<?php echo $option['id']; ?>" <?php if($option['id'] == $idSucursal)  echo "selected"; ?> >
...
?>><?php echo $option['NombreComercial']; ?></option>
                            ...
                            <div class="col-lg-3">
                              ...
                              <input type="datetime-local" class="form-control" name="txtFechaGeneracion" id="txtFechaGeneracion"
                                value="<?=$FechaGeneracionF?>" readonly required>
                            </div>
                            <div class="col-lg-3">
                              <?php
...
?>
                              ...
                              <input type="date" class="form-control" name="txtFechaVencimiento"
                                id="txtFechaVencimiento" value="<?=$FechaVencimientoOr?>" required  min="<?=$FechaGeneracionCampo?>" max="<?=$fechaVencimientoF?>">
                            </div>
                          </div>
                          <br>
                                ...
                                <div class="col-lg-12 input-group">
                                  <span class="input-group-addon" style="width:70%">
                                    <select name="cmbProducto" id="chosenProducto" style="width: 90%;" required>
                                      <option value="">Elegir opción</option>
                                        <?php
                                        $stmt = $conn->prepare('SELECT p.PKProducto,p.Nombre,p.ClaveInterna FROM productos as p INNER JOIN tipos_productos as tp ON p.FKTipoProducto = tp.PKTipoProducto LEFT JOIN rel_acciones_producto as rap ON rap.FKProducto = p.PKProducto WHERE rap.FKAccionProducto = 2 AND p.empresa_id = :idempresa AND p.estatus = 1 ORDER BY p.Nombre ASC');
                                        $stmt->bindValue("idempresa",$_SESSION['IDEmpresa']);
                                        $stmt->execute();
                                      ?>
                                      <?php foreach ($stmt as $option): ?>
                              ...
                              <div>
                                <div class="row" id="divCantidad">
                                  ...
                                  <div class="col-lg-6">
                                    ...
                                    <input type='text' value='' name="txtPrecio" id="txtPrecio" class='form-control txtPrecio'
...
foreach ($rowp as $rp) {
    ...
    "<input type='hidden' name='inp_total_producto[]' value='" . number_format($totalProducto, 2) . "' />" .
        "<th><button type='button' class='btn eliminarProductos' id='" . $rp['FKProducto'] . "'><img src='../../img/timdesk/delete.svg' width='20px' /></button></th>" .
        "</tr>";
}
...
?>
  ...
  $("#agregarProducto").click(function() {
    ...
    if (Precio == null || isNaN(Piezas) || Precio == 0 ) {
      $("#alertaPrecio").css("display", "block");
      setTimeout(function() {
        $("#alertaPrecio").css("display", "none");
      }, 2000);
      return;
    }
