  ...
  if(isset($_SESSION["Usuario"]) && ($_SESSION["FKRol"] == 4 || $_SESSION["FKRol"] == 1)){
    require_once('../../../../../include/db-conn.php');
      if(isset ($_POST['btnEditar'])){
        ...
        try{
        $stmt = $conn->prepare('UPDATE cuentas_bancarias_paqueterias set FKBanco= :banco,No_de_cuenta= :cuenta, CLABE= :clabe WHERE PKCuentaPaqueteria = :id');
      ...
      if(isset($_POST['idCuentaU'])){
        $id =  $_POST['idCuentaU'];
        $stmt = $conn->prepare('SELECT * FROM cuentas_bancarias_paqueterias WHERE PKCuentaPaqueteria= :id');
