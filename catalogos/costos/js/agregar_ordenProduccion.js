...
$(document).ready(function(){
  ...
  $("#txtCantidad").on("keyup",function(){
    ...
    $("#tblMateriales tbody tr").each(function(i){
      ...
      currentRow.find("td:eq(4)").text(cantidad * cantidad_a_consumir[i])
      
      var id = currentRow.find("td:eq(0)").text();
      a_consumir_table = parseInt(currentRow.find("td:eq(4)").text());
...
$(document).on("change","#cmbProdcuto",function(){
  if($(this).hasClass("is-invalid")){
...
$(document).on("click","#add_lote",function(){
  ...
  if(lotes_existencia.length > 0){
    lotes_existencia.forEach(function(i){
      if(i.id === material_id && i.cantidad){
        if(i.cantidad !== 0){
  ...
  if(ban){
    ...
    if(parseInt(aux[2]) >= parseInt(cantidad)){
      //if(lotes_existencia.length > 0){
        const lote_void = lotes_existencia.find( i => i.lote === "");
        const data = lotes_existencia.find( i => i.lote === aux[1]);
        const id_find = lotes_existencia.find( i => i.id === aux[0]);
            ...
            (
              {
                ...
                //a_consumir: falta llenar este valor desde la tabla por producto
              }
            );
        } else {
...
$(document).on("click","#tblMaterialsLot .delete_lot",function(){
  ...
  removeItemFromArrLot(lotes_existencia,val1);
});
...
function removeItemFromArrLot ( arr, item ) {
