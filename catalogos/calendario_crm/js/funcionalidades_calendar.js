...
function initCalendar() {
  ...
  if (meet) {
    var dateFormat = new Date(parseInt(meet));
    calendar.gotoDate(dateFormat);
    //var res = removeParam("meet", url.href);
    //console.log({ res });
...
$(document).ready(function () {
  ...
  $("#opciones-usuario").on("change", function () {
    ...
    if (filter_option_all_events == 1) {
      ...
      $.ajax({
        url: "app/modelos/loadCalendario.php",
        method: "POST",
        data: {
          accion: accion,
        },
      ...
      var accion = "cargar_eventos_contacto";
      $.ajax({
        url: "app/modelos/loadCalendario.php",
        method: "POST",
        data: {
...
function buscarEmpleados() {
  ...
  console.log({ idContacto: evento.contacto_id, idEvento: info_evento.id });
  $.ajax({
    url: "app/controladores/loadCalendarController.php",
    data: {
      accion: accion,
...
function actividadTres(evento, info_evento) {
  ...
  var fecha_tarea = moment(info_evento.start).format("YYYY-MM-DD");
  $("#fecha").val(fecha_tarea);
  $("#fecha_llamada-2").val(fecha_tarea);
  var fecha_es = crearFecha(fecha_tarea);
  $("#label-date-llamada").text(fecha_es);
...
function enviarCorreoEdicion(color, title, accion, actividad, contacto_id, id) {
  ...
  let datosActividad = { color, title, accion, actividad, contacto_id, id };
  console.log({ datosReunion, datosActividad });
...
function camposEditarReunionCorreo(
  ...
  console.log({ color, title, accion, actividad, contacto_id, id, confirm });
