  ...
  class save_data{
    function savePermission($value,$user){
      ...
      try {
        ...
        $query = sprintf('SELECT pp.PKPermisoPantalla FROM permisos_pantallas pp WHERE pp.FKUsuario = ?');
        $stmt = $db->prepare($query);
        $stmt->execute(array($user));
        $countPerPan = $stmt->rowCount();

        $query = sprintf('SELECT ps.PKPermisoSeccion FROM permisos_secciones ps WHERE ps.FKUsuario = ?');
        $stmt = $db->prepare($query);
        $stmt->execute(array($user));
        $countPerSec = $stmt->rowCount();

        $query = sprintf('SELECT pf.PKPermisoFuncion FROM permisos_funciones pf WHERE pf.FKUsuario = ?');
        $stmt = $db->prepare($query);
        $stmt->execute(array($user));
        $countPerFun = $stmt->rowCount();

        /* Secciones*/
        $query = sprintf("SELECT PKSeccion FROM secciones");
        $stmt = $db->prepare($query);
        $stmt->execute();
        ...
        foreach($arraySection as $r){
          if(!in_array($r['PKSeccion'], $auxSecciones)){
            if($r['PKSeccion'] !== 5){
              $secciones[$contador]['seccion'] = $r['PKSeccion'];
        ...
        $query = sprintf("SELECT PKPantalla,FKSeccion FROM pantallas");
        $stmt = $db->prepare($query);
        $stmt->execute();
        $arrayScreen = $stmt->fetchAll();

        foreach ($arrayScreen as $r) {
          if(!in_array($r['PKPantalla'], $auxPantallas)){
            if($r['FKSeccion'] === 5){
              $pantallas[$contador]['pantalla'] = $r['PKPantalla'];
        ...
        $query = sprintf("SELECT PKFuncion,PKSeccion FROM funciones 
                          INNER JOIN pantallas ON FKPantalla = PKPantalla
                          INNER JOIN secciones ON FKSeccion = PKSeccion");
        $stmt = $db->prepare($query);
        $stmt->execute();
        $arrayFunction = $stmt->fetchAll();

        foreach ($arrayFunction as $r) {
          if(!in_array($r['PKFuncion'], $aux2)){
            if($r['PKSeccion'] === 5){
              $funciones[$contador]['funcion'] = $r['PKFuncion'];
