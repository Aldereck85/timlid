...
require_once('function_formatoCantidad.php');
  ...
  if($fecha_desde!="no"){
    ...
    $between='and vd.created_at between "'.$fecha_desde.'" and "'.$fecha_hasta.' 23:59:59"';
}else{
    if($fecha_hasta!="no"){
      $between='and f.fecha_timbrado <= "'.$fecha_hasta.' 23:59:59"';
      $between='and vd.created_at <= "'.$fecha_hasta.' 23:59:59"';
    }
}
  ...
  if($seleccion==1){
    $stmt = $conn->prepare('SELECT * from(
      SELECT c.PKCliente, 
            ...
            where f.cliente_id=c.PKCliente and c.PKCliente=:cliente and datediff(if(f.fecha_vencimiento is not null,f.fecha_vencimiento, date_add(f.fecha_timbrado, interval c.Dias_credito day)), SYSDATE()) > 90 and f.empresa_id=:idEmpresa and f.estatus  in (1,2) '.$between.'
    ...
    $stmt = $conn->prepare('SELECT * from(
      SELECT c.PKCliente, 
            ...
            left join (select sum(nc.importe) as importeNC, ncf.facturacion_id, group_concat(concat(nc.num_serie_nota," ", nc.folion_nota) SEPARATOR "/") as foliosNotas 
                ...
                inner join notas_cuentas_por_cobrar_has_facturacion as ncf on nc.id = ncf.notas_cuentas_por_cobrar_id 
                where nc.estatus = 1 and nc.empresa_id=:empresa2 group by ncf.facturacion_id) as notas on notas.facturacion_id = f.id
            where f.cliente_id=c.PKCliente and c.PKCliente=:cliente and datediff(SYSDATE(), if(f.fecha_vencimiento is not null,f.fecha_vencimiento, date_add(f.fecha_timbrado, interval c.Dias_credito day))) > 90 and f.empresa_id=:idEmpresa and f.estatus  in (1,2) '.$between.'
            ...
            left join (select sum(nc.importe) as importeNC, ncf.venta_id, group_concat(concat(nc.num_serie_nota," ", nc.folion_nota) SEPARATOR "/") as foliosNotas 
              from notas_cuentas_por_cobrar as nc 
                  inner join notas_cuentas_por_cobrar_has_ventas as ncf on nc.id = ncf.notaCredito_id 
              where nc.estatus = 1 and nc.empresa_id=:idEmpresa3 group by ncf.venta_id) as notas on notas.venta_id = vd.PKVentaDirecta          
    where vd.FKCliente=c.PKCliente and c.PKCliente=:cliente2 and datediff(SYSDATE(), if(vd.FechaVencimiento is not null,vd.FechaVencimiento, date_add(vd.created_at, interval c.Dias_credito day))) > 90 and vd.empresa_id=:idEmpresa2 and vd.empresa_id !=6 and vd.estatus_factura_id not in (1,2) and vd.FKEstatusVenta != 5 and vd.estatus_cuentaCobrar in (1,2) '.$betweenb.'
                ) as tabla Order by "Fecha de facturacion" desc;');    
  }
      ...
      $stmt = $conn->prepare('SELECT * from (
        SELECT c.PKCliente, 
          ...
          left join (select sum(nc.importe) as importeNC, ncf.facturacion_id, group_concat(concat(nc.num_serie_nota," ", nc.folion_nota) SEPARATOR "/") as foliosNotas 
              ...
              inner join notas_cuentas_por_cobrar_has_facturacion as ncf on nc.id = ncf.notas_cuentas_por_cobrar_id 
              where nc.estatus = 1 and nc.empresa_id=:empresa2 group by ncf.facturacion_id) as notas on notas.facturacion_id = f.id
          where f.cliente_id=c.PKCliente and c.PKCliente=:cliente and datediff(if(f.fecha_vencimiento is not null,f.fecha_vencimiento, date_add(f.fecha_timbrado, interval c.Dias_credito day)), SYSDATE()) between :_periodo and :periodo and f.empresa_id=:idEmpresa and f.estatus  in (1,2) and f.prefactura = 0 '.$between.'
            ...
            left join (select sum(nc.importe) as importeNC, ncf.venta_id, group_concat(concat(nc.num_serie_nota," ", nc.folion_nota) SEPARATOR "/") as foliosNotas 
              from notas_cuentas_por_cobrar as nc 
                  inner join notas_cuentas_por_cobrar_has_ventas as ncf on nc.id = ncf.notaCredito_id 
              where nc.estatus = 1 and nc.empresa_id=:idEmpresa3 group by ncf.venta_id) as notas on notas.venta_id = vd.PKVentaDirecta          
    where vd.FKCliente=c.PKCliente and c.PKCliente=:cliente2 and datediff(if(vd.fechaVencimiento is not null,vd.fechaVencimiento, date_add(vd.created_at, interval c.Dias_credito day)), SYSDATE()) between :_periodo2 and :periodo2 and vd.empresa_id=:idEmpresa2 and vd.empresa_id !=6 and vd.estatus_factura_id not in (1,2) and vd.FKEstatusVenta != 5 and vd.estatus_cuentaCobrar in (1,2) '.$betweenb.'
      ...
      $stmt = $conn->prepare('SELECT * from (
        SELECT c.PKCliente, 
          ...
          left join (select sum(nc.importe) as importeNC, ncf.facturacion_id, group_concat(concat(nc.num_serie_nota," ", nc.folion_nota) SEPARATOR "/") as foliosNotas 
              ...
              inner join notas_cuentas_por_cobrar_has_facturacion as ncf on nc.id = ncf.notas_cuentas_por_cobrar_id 
              where nc.estatus = 1 and nc.empresa_id=:empresa2 group by ncf.facturacion_id) as notas on notas.facturacion_id = f.id
          where f.cliente_id=c.PKCliente and c.PKCliente=:cliente and datediff(SYSDATE(), if(f.fecha_vencimiento is not null,f.fecha_vencimiento, date_add(f.fecha_timbrado, interval c.Dias_credito day))) between :_periodo and :periodo and f.empresa_id=:idEmpresa and f.estatus  in (1,2) '.$between.'
            ...
            left join (select sum(nc.importe) as importeNC, ncf.venta_id, group_concat(concat(nc.num_serie_nota," ", nc.folion_nota) SEPARATOR "/") as foliosNotas 
              from notas_cuentas_por_cobrar as nc 
                  inner join notas_cuentas_por_cobrar_has_ventas as ncf on nc.id = ncf.notaCredito_id 
              where nc.estatus = 1 and nc.empresa_id=:idEmpresa3 group by ncf.venta_id) as notas on notas.venta_id = vd.PKVentaDirecta          
    where vd.FKCliente=c.PKCliente and c.PKCliente=:cliente2 and datediff(SYSDATE(), if(vd.FechaVencimiento is not null,vd.FechaVencimiento, date_add(vd.created_at, interval c.Dias_credito day))) between :_periodo2 and :periodo2 and vd.empresa_id=:idEmpresa2 and vd.estatus_factura_id not in (1,2) and vd.FKEstatusVenta != 5 and vd.estatus_cuentaCobrar in (1,2) '.$betweenb.'
