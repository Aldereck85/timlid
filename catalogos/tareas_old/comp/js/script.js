	...
	$.ajax({
		...
		success:function(resp){
			...
			$.each(resp, function(i){
				...
				$('#boardContent').append('<div id="etapa-'+this.PKEtapa+'" class="container-fluid"><div class="contenedor"><div class="encabezado-grupo titulo-etapa estilo-etapa">'+this.Etapa+'<button class="btnColorPicker" style="float:right;">&nbsp;</button></div><div id="columnas-principales-etapa-'+this.PKEtapa+'" class="etapas order_'+(i+1)+' d-flex"></div</div>');
...
function getTask(id){ //Obtener las tareas del proyecto
	...
	$.ajax({
		...
		success:function(resp){
			...
			$.each(resp, function(i){
				$('#tabla-id-'+resp[i].FKEtapa).append("<div id='tarea-"+this.PKTarea+"' class='contenedor' data-ord="+this.PKTarea+"><div class='encabezado-etapa titulo-item ml-3'><div id='rcorners1'>&nbsp;&nbsp;&nbsp;</div>&nbsp;"+this.Tarea+"</div></div>");
