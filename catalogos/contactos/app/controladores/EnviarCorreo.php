...
class EnviarCorreoContactos
{
    ...
    public static function store($agregar_empleados, $actividad_id, $data)
    {
        ...
        foreach ($agregar_empleados as $empleado_agregar) {
            ...
            $email_body = "<b>Tiene una invitación de reunión:</b>" . $data['fecha'] . "<br>
                      <b>Nombre de la invitacion:</b>" . $result["title"] . "<br>
                      <b>Fecha:</b>" . $data['fecha'] . "<br>
                      <b>Hora:</b>" . $data['hora_inicio'] . "<br>
                      <b>Lugar de la reunión:</b>" . $data['lugar'] . "<br>
                      <b>Asistentes:</b>" . $result["participantes_reunion"] . "<br>
                      <b>Convocante:</b>" . $data['username'];
    ...
    public static function update($data, $opc)
    {
        ...
        if ($stmt->execute()) {
            foreach ($data[0] as $empleado) {
                ...
                $stmt->execute();
                $resulted = $stmt->fetch();
                self::enviarCorreo1($resulted, $email_body);
            }
        }
    }
    ...
    public static function reenviarCorreos($data)
    {
        $con = new ConnDB();
        $conn = $con->getDb();
        ...
        foreach ($data['json_empleados'] as $idUsuario) {
            $stmt = $conn->prepare("INSERT INTO envios_correos_temp(empleado_id, actividad_id) VALUES (:empleado_id,:actividad_id)");
            $stmt->bindValue(':empleado_id', $idUsuario);
            $stmt->bindValue(':actividad_id', $actividad_id);
            $stmt->execute();
        }

        self::store($data['json_empleados'], $actividad_id, $data);
    }
}
