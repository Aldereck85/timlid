...
function setnotification($conn, $idelemento)
{
  ...
  foreach ($empleados as $empleado) {
    ...
    $stmt = $conn->prepare('INSERT INTO notificaciones (tipo_notificacion, detalle_tipo_notificacion, id_elemento, created_at, usuario_creo, usuario_recibe) VALUES (:tipoNot, :detaleNot, :idElem, :fecha, :usrCreo, :usrRecibe)');
    $stmt->execute([':tipoNot' => 6, ':detaleNot' => 20, ':idElem' => $idelemento, ':fecha' => $timestamp, ':usrCreo' => $_SESSION['PKUsuario'], ':usrRecibe' => $empleado['id']]);
  }
}
