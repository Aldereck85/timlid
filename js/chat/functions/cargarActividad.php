...
if(isset($_SESSION["Usuario"])){
    ...
    $stmt = $conn->prepare('SELECT ca.Tipo, ca.Fecha, IFNULL(CONCAT(e.Nombres," ", e.PrimerApellido), u.Nombre) as nombreempleado, cc1.nombre as columnaorigen, cc1.color as columnaorigencolor, cc2.nombre as columnadestino, cc2.color as columnadestinocolor FROM chat_actividad as ca INNER JOIN usuarios as u ON u.id = ca.FKUsuario LEFT JOIN empleados_usuarios as eu ON eu.FKUsuario = u.id LEFT JOIN empleados as e ON e.PKEmpleado = eu.FKEmpleado LEFT JOIN colores_columna as cc1 ON cc1.PKColorColumna = ca.FKColorColumnaOrigen LEFT JOIN colores_columna as cc2 ON cc2.PKColorColumna = ca.FKColorColumnaDestino WHERE ca.FKTarea = :idtarea ORDER BY PKChat_actividad Desc');
    ...
    if($nc > 0){
      ...
      '<div class="panel panel-default">'.
        '<div class="panel-body">'.
          '<div class="col-md-12">'.
            '<div class="btn-group">'.
                '<span><img src="'.$ruta.'../../img/chat/filtrar_actividad.svg" class="img-responsive" width="15px">'.
                  '<span class="panel-header">Filtros</span>'.
                    '<button type="button" id="filtros" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><img src="'.$ruta.'../../img/chat/filtros_expandir.svg" class="img-responsive" width="15px" style="position: relative; left: 20px;"></button>'.
                          ...
                          '<div class="col-md-4">'.
                            ...
                            '<div class="col-md-4">'.
                              ...
                              foreach ($filtros as $f) {
                                 $html .= '<button type="button" class="btn-mdb btn-blue-grey btn-filter-etapa btn-sm-mdb" data-target="'.$f['columnadestino'].'" style="width: 90%;">'.$f['columnadestino'].'</button>';
                              }
                  ...
                  $html .= 
                      ...
                      '<td width="30%">'. 
                        '<div class="row izquierdo">'.
                              '<img src="'.$ruta.'../../img/chat/calendario.svg" width="20px" class="img-responsive calendario tooltip_chat" data-toggle="tooltip" title="" data-original-title="'.$fecha.'" />'. 
                              '<span style="width:35px;text-align: center;">'.$tiempo."</span>".
                              '<span data-toggle="tooltip" class="tooltip_chat" data-original-title="'.$r['nombreempleado'].'">'.
                                '<img src="'.$ruta.'../../img/chat/users.svg" class="user-img img-responsive usuario-tabla" width="33px" />'.
                              '</span>'.
                        '</div>'.
                      '</td>'.
                        ...
                        if($r['Tipo'] == 1){
                          ...
                          '<td width="10%">'.
                              '<div class="estado-tabla" align="center">'.
                                '<div>'.
                                  '<img src="'.$ruta.'../../img/chat/cambio_estado.svg" class="img-responsive" width="20px" height="20px" style="margin-top: 3px;">'.
                                '</div>'.
                                'Estado'.
                              '</div>'.
                          '</td>'.
                          '<td width="28%">'.
                              '<div class="fases-tabla" style="width: 100%;text-align: center;">'.
                                '<div class="estado-actividad" title="" style="background-color:'.$r['columnaorigencolor'].' !important;">'.$r['columnaorigen'].'</div>'.
                              '</div>'.
                          '</td>'.
                          '<td width="4%">'.
                          ...
                          '<td width="28%">'.
                            '<div class="fases-tabla" style="width: 100%;text-align: center;">'.
                                '<div class="estado-actividad" title="" style="background-color:'.$r['columnadestinocolor'].' !important;">'.$r['columnadestino'].'</div>'.
                            '</div>'.
                          '</td>';
                        }
                        else{
          ...
          $html = '<br><br>
                  <div class="row">
                    <div class="col-md-12">
                        <center><span>AÚN NO HAY NINGUNA ACTIVIDAD</span></center>
                    </div>
                  </div>';
        }
