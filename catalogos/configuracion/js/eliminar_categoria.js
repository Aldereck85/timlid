    ...
    $.ajax({
        ...
        success:function(respuesta){
          ...
          if(respuesta[0].status){
            ...
            $('#tblCategoriadeProductos').DataTable().ajax.reload(),3000;
          }else{
            Lobibox.notify('fail', {
              size: 'mini',
              rounded: true,
...
function eliminarCategoriaCliente(){
  var id = $("#txtPKCategoriaClienteD").val();

  $.ajax({
      url:"php/funciones.php",
      data:{clase:"delete_data", funcion:"delete_categoriaCliente", datos:id},
      dataType:"json",
      success:function(respuesta){  
        if(respuesta[0].status){
          Lobibox.notify('success', {
          ...
          $('#tblCategoriadeClientes').DataTable().ajax.reload(),3000;
