...
$stmt = $conn->prepare("SELECT distinct ispp.folio_salida as folioSalida,
                          ...
                          ispp.folio_salida,
                          sd.calle as CalleE,
                          sd.numero_exterior as NumExtE,
                          sd.numero_interior as NumIntE,
                          sd.colonia as ColoniaE,
                          sd.municipio as MunicipioE,
                        ...
                        from inventario_salida_por_sucursales ispp
                          ...
                          inner join sucursales sd on opps.sucursal_destino_id = sd.id
                          inner join paises psE on sd.pais_id = psE.PKPais
                          inner join estados_federativos efE on sd.estado_id = efE.PKEstado
                          inner join empresas em on so.empresa_id = em.PKEmpresa and sd.empresa_id = em.PKEmpresa
                          left join empleados e on ispp.surtidor_id = e.PKEmpleado
                          left join empleados eOP on ispp.usuario_creo_id = eOP.PKEmpleado
                          inner join inventario_entrada_por_sucursales ieps on ieps.orden_pedido_id = opps.id
...
$rowUs = $stmtUs->fetch();
  ...
  $GLOBALS["FolioEntrada"] = $row['folio_entrada'];
...
class PDF extends TCPDF {
    ...
    public function Header() {
      ...
      $this->MultiCell(0, 30, 'Traspaso', 0, 'C', 0, 0, 0, 10, true);
      ...
      // set cell margins
      $this->setCellMargins(1, 1, 1, 1);
      $this->MultiCell(70, 5, 'Información del traspaso', 1, 'C', 0, 1, 210, 10, true);
      $this->MultiCell(70, 5, str_pad($GLOBALS["FolioOrden"], 10, "0", STR_PAD_LEFT), 1, 'C', 0, 1, 210, 16, true);
      $this->MultiCell(70, 5, 'Folio de salida', 1, 'C', 0, 1, 210, 22, true);
      $this->MultiCell(70, 5, ''.$GLOBALS["FolioSalida"], 1, 'C', 0, 1, 210, 28, true);
      $this->MultiCell(70, 5, 'Folio de entrada', 1, 'C', 0, 1, 210, 34, true);
      $this->MultiCell(70, 5, $GLOBALS["FolioEntrada"], 1, 'C', 0, 1, 210, 40, true);
    }
...
<table>
  <tr>
    <td class="td3 td4" width="15%">Sucursal de destino: </td>
...
foreach ($rowp as $rp) { 
  ...
  $tbl.=  '<tr>
            ...
            <td width="21%">'.$rp['unidadMedida'].'</td>
...
$pdf->Output("Traspaso ".$GLOBALS["FolioSalida"]. '.pdf', 'D');
return true;
?>
