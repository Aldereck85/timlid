...
$stmtp = $conn->prepare("SELECT id, producto, nombre, clave, cantidad, unidadMedida, precio, importe, GROUP_CONCAT(impuestos SEPARATOR ' / ') as impuestos  from (
  ...
  from ordenes_compra oc
    ...
    inner join datos_producto_proveedores dpp on doc.FKProducto = dpp.FKProducto and (select FKProveedor from ordenes_compra where PKOrdenCompra = :idOrdenCompra1) = dpp.FKProveedor
    inner join productos p on doc.FKProducto = p.PKProducto  
  ...
  where doc.FKOrdenCompra = :idOrdenCompra2
  ) as tabless
   GROUP BY producto 
   ;");
...
$pdfdoc = $pdf->Output("Orden de Compra". $GLOBALS["referencia"], "S");
...
try {    
    ...
    $mail->addReplyTo($origen, "Contacto");
    ...
    $mail->Body    = utf8_decode($mensaje.'<br> <a href="https://app.timlid.com/catalogos/inventarios_productos/catalogos/orden_compras/aceptarOrdenCompra.php?oc='.md5($id).'" style="background-color: #006dd9; /* TIMLID */
    border: none;
    color: white;
    padding: 15px 32px;
    text-align: center;
