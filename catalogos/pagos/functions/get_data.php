...
$id = ($_GET['user_id']);
$stmt = $conn->prepare("SELECT dtcpp.id,dtcpp.clave, pd.Nombre, dtcpp.cantidad,dtcpp.precio,
dtcpp.descuento,dtcpp.iva,dtcpp.ieps,dtcpp.ieps_monto_fijo,pr.NombreComercial FROM proveedores as pr  
...
inner join productos as pd on
pd.ClaveInterna = dtcpp.clave
WHERE cp.id= $id and pr.empresa_id = $empresa");
$stmt->execute();
