$(document).ready(function(){
    $("#tblListadoTipoProducto").dataTable({
      "lengthChange": false,
      "pageLength": 15,
      //"paging": true,
      "info": false,
      ...
      "ajax": {
        url:"../../php/funciones.php",
          data:{clase:"get_data", funcion:"get_tipoProductoTable"},
      },
      "order": [ 1, 'asc' ],
      "columns":[
        { "data": "Id" },
      ...
      "language": setFormatDatatables(),
        ...
        rowCallback: function(row, data) {
          ...
          if ((data.Estatus.substr(25,8) == 'Inactivo')) {
            ...
            $($(row).find("td label.textTable")[0]).attr("title", "Tipo producto inactivo");
          } else {
            $($(row).find("td")[0]).css("background-color", "#28c67a");
            $($(row).find("td label.textTable")[0]).attr("style", "color: #FFFFFF!important");
            $($(row).find("td label.textTable")[0]).attr("title", "Tipo producto activo");
          }
        }
    });
  });
