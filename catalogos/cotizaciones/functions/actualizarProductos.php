...
if (isset($_SESSION["Usuario"])) {
    ...
    if($_POST['actualizar'] == 0){
        ...
        $stmt = $conn->prepare('SELECT p.PKProducto,p.Nombre,p.ClaveInterna FROM productos as p LEFT JOIN operaciones_producto as op ON p.PKProducto = op.FKProducto WHERE op.Venta = 1 AND p.empresa_id = :idempresa AND p.estatus = 1 ORDER BY p.Nombre ASC');
        $stmt->bindValue("idempresa",$_SESSION['IDEmpresa']);
        $stmt->execute();
