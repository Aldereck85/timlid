 ...
 */
 var arrProdID = {};
 //objeto con los productos iniciales, para comparación
 var arrProdOlds = {};
 //objetos con los datos de productos a eliminar y editar
 var toInsert = {};
 var toDelete = {};
 var toUpdate = {};

 function isEditable(idRequisicion){
  $.ajax({
 ...
 //función para convertir la cadena de productos recuperada de la requisición al objeto
 function cadenaToArr(cadena){
  ...
  productos.forEach(element => {
    ...
    arrPropiedadesProd["id"] = aux[0];
    arrPropiedadesProd["cantidad"] = aux[1];
    arrPropiedadesProd["nombre"] = aux[2];
    arrPropiedadesProd["clave"] = aux[3];

    arrProdID[aux[0]] = Object.assign({}, arrPropiedadesProd);
    arrProdOlds[aux[0]] = Object.assign({}, arrPropiedadesProd);
    AgregaProductos();
  });
 }

 function recuperaDatosEdit(idRequisicion){
    $.ajax({
        url: "php/functions.php",
        data: { 
            clase: "get_data", 
            funcion: "get_dataRequisicionEdit",
            data: idRequisicion
        },
        dataType: "json",
        success: function (respuesta) {
            if(respuesta["estatus"] == "ok"){
              $("#txtFechaEmision").val(respuesta['cabecera'][0].fecha_registro);
              $("#txtFechaEstimada").val(respuesta['cabecera'][0].fecha_estimada_entrega);
              $("#txtFechaEstimada").prop("min", respuesta['cabecera'][0].fecha_estimada_entrega);
              cargarCMBSucursal(respuesta['cabecera'][0].FKSucursal, "cmbDireccionEnvio");
              cargarCMBArea(respuesta['cabecera'][0].area,"cmbArea");
              cargarCMBEmpleado(respuesta['cabecera'][0].aplicado_por,"cmbEmpleado");
              cargarCMBComprador(respuesta['cabecera'][0].comprador,"cmbComprador");

              if(respuesta['cabecera'][0].FKProveedor != "" && respuesta['cabecera'][0].FKProveedor != null && respuesta['cabecera'][0].FKProveedor != 0){
                proveedor = (respuesta['cabecera'][0].FKProveedor);
                cargarCMBProveedor(proveedor, "cmbProveedor");
              }else{
                cargarCMBProveedor("", "cmbProveedor");
                cmbTodosLosProductos();
              }

              $("#NotasComprador").val(respuesta['cabecera'][0].notas_comprador);
              $("#NotasInternas").val(respuesta['cabecera'][0].notas_internas);

              cadenaToArr(respuesta['cadena']);
            }else{
 ...
 function validaFormulario(isSaveAll=0){
   ...
   if ($("#frmRequisicionCompraEdit")[0].checkValidity()) {
 ...
 //función para contruir arrays con los productos a insertar y eliminar de la requisición
 function constructArrays() {
  $("#btnAgregar").prop("disabled", true);
  $("#agregarProducto").prop("disabled", true); 

  if (!$.isEmptyObject(arrProdID)) {
    toInsert = Object.assign({}, arrProdID);//arreglo actualizado
    toDelete = Object.assign({}, arrProdOlds);//arreglo inicial
    toUpdate = Object.assign({}, arrProdID);//arreglo actualizado

    //se eliminan los productos que estan en ambos arreglos para determinar aquellos que se van a añadir, eliminar y actualizar
    for (const property in arrProdOlds) {
      if (arrProdID.hasOwnProperty(property)) {
        delete toInsert[property];
        delete toDelete[property];

        //comprueba si cambió la cantidad, nombre o clave, si no, se elimina del arreglo toUpdate
        if(
          arrProdID[property]['cantidad'] === arrProdOlds[property]['cantidad'] &&
          arrProdID[property]['cantidad'] === arrProdOlds[property]['cantidad'] &&
          arrProdID[property]['cantidad'] === arrProdOlds[property]['cantidad']
          ){
     ...
     success: function (respuesta) {
       ...
       $.each(respuesta, function (i) {
        if(data === respuesta[i].PKEmpleado){
         ...
         html +=
           `<option value="${respuesta[i].PKEmpleado}" ${selected}>${respuesta[i].NombreCompleto}</option>`;
       });
     ...
     success: function (respuesta) {
       ...
       $.each(respuesta, function (i) {
        if(data === respuesta[i].id){
         ...
         html +=
           `<option value="${respuesta[i].id}" ${selected}>${respuesta[i].nombre}</option>`;
       });
...
//actualiza la cantidad en el arreglo de productos
 ...
 $(document).ready(function () {
    var idRequisicion = $("#inpt_idRequisicion").val();
    isEditable(idRequisicion);
    resetValidations();
    Slimselects();
    recuperaDatosEdit(idRequisicion);
   ...
   $("#alertEditable").on("hidden.bs.modal", function (e) {
    window.location.href = "../../../dashboard.php";
   });
});