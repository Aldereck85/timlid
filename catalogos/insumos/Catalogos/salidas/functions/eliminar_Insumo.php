...
session_start();
    ...
    if(isset($_SESSION["Usuario"])){
      ...
      try{
        $stmt = $conn->prepare('call spd_EliminarSalidaUnica(:id,:usuario)');
        $stmt->bindValue(':id',$id,PDO::PARAM_INT);
        $stmt->bindValue(':usuario',$usuario,PDO::PARAM_STR);
        /*$stmt->bindValue(':id', $id, PDO::PARAM_INT);*/
        if($stmt->execute()){
          $_SESSION['message'] = 'Operación exitosa. Salida eliminada correctamente.';
          $_SESSION['message_type'] = 'success';

          header("Location: ../");
        }else{
          echo "fallo";
          $_SESSION['message'] = 'Operación fallida. Salida no eliminada correctamente.';
