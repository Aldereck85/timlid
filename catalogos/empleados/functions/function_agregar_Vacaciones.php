...
session_start();
  ...
  $json = new \stdClass();
    ...
    if($bandera){
      ...
      try{
        $stmt = $conn->prepare('INSERT INTO vacaciones (FKEmpleado, Dias_de_Vacaciones_Tomados, FechaIni, FechaFin, Prima_vacacional, Total_Vacaciones) values (:FKEmpleado, :dias_vacaciones,:fechaini, :fechafin, :prima_vacacional,:total_vacaciones)');
        ...
        $stmt->bindValue(':prima_vacacional',$Prima_vacacional);
        $stmt->bindValue(':total_vacaciones',$total_vacaciones);
        $stmt->execute();
        ...
        $json->sueldo_vacaciones = number_format($total_vacaciones - $Prima_vacacional,2,'.','');
