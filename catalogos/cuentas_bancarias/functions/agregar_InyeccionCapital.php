...
if (isset($_SESSION["Usuario"])) {
    ...
    if ($hayArchivo == 1) {
        ...
        if (move_uploaded_file($_FILES['inputFileInyeccion']['tmp_name'], $location)) {
            try {
                $stmt = $conn->prepare('INSERT INTO movimientos_cuentas_bancarias_empresa (cuenta_origen_id, Fecha, tipo_movimiento_id, Descripcion, Deposito, Referencia, Comprobado, Saldo)
                VALUES (:fkcuenta, :fecha, :tipo, :descripcion, :deposito, :referencia, :comprobado, 0)');
           ...
           INNER JOIN movimientos_cuentas_bancarias_empresa as mcb ON cc.FKCuenta=mcb.cuenta_origen_id WHERE mcb.PKMovimiento =:idLast');
                ...
                if ($uCuenta === true && $exitoMov === true && $uMov === true) {
                    echo "exito";
                } else {
                    echo "fallo";
                }
