...
if ($toggle == 1) {
  ...
  if ($periodo == "00") {
    ...
    cp.fecha_vencimiento,cp.estatus_factura, pr.NombreComercial, cp.saldo_insoluto FROM proveedores as pr  
    ...
    WHERE (cp.estatus_factura = $status or 2) and cp.proveedor_id= $proveedor_id and (pr.empresa_id = $empresa) and 
    ...
    $stmt = $conn->prepare("SELECT cp.id, cp.proveedor_id, tipo_estatus, cp.folio_factura, DATEDIFF(SYSDATE(), fecha_vencimiento) as vencimiento,
     cp.num_serie_factura, cp.subtotal, cp.importe, cp.fecha_factura, cp.saldo_insoluto,
  ...
  if ($periodo == "00") {
    ...
    left JOIN  cuentas_por_pagar as cp ON cp.proveedor_id = pr.PKProveedor and cp.estatus_factura!=7
    left join estatus_factura as stat on cp.estatus_factura = stat.id
    WHERE  cp.proveedor_id= $proveedor_id and (pr.empresa_id = $empresa) and DATEDIFF(SYSDATE(), $toggle) >= 90 GROUP BY cp.id ");
    /*  */
    /* Si NO quiere decir que el periodo se tiene que evaluar para saber por donde comenzar el rango */
    /*  */
  } else {
    ...
    cp.subtotal, cp.importe, cp.fecha_factura,cp.fecha_vencimiento,cp.estatus_factura, pr.NombreComercial, cp.saldo_insoluto 
    FROM proveedores as pr  
    left JOIN  cuentas_por_pagar as cp ON cp.proveedor_id = pr.PKProveedor and cp.estatus_factura!=7
    left join estatus_factura as stat on cp.estatus_factura = stat.id
    WHERE  cp.proveedor_id= $proveedor_id and DATEDIFF(SYSDATE(), $toggle) 
    between $_periodo  and $periodo and (pr.empresa_id = $empresa) GROUP BY cp.id ");
  }
}
