...
if (isset($_SESSION["Usuario"])) {
    ...
    /* if ($tipoCambio != "") {
    $cantidadAgregar = $cantidadDisponer;
    $cantidadQuitar = $cantidadDisponer * $tipoCambio;
    } else {
    $cantidadAgregar = $cantidadDisponer;
    $cantidadQuitar = $cantidadDisponer;
    } */
    $cantidadAgregar = $cantidadDisponer;
    $cantidadQuitar = $cantidadDisponer;
    ...
    try {
        ...
        if ($hayCuenta == 1) { //IF exist account
            if ($tipoC == 2) {
                ...
                $saldoFU = $saldoAnterior + $cantidadAgregar;
                ...
                $credDisponible = $limCredito - $credDisponible;

                //UPDATE DE LA CUNETA CREDITO DESTINO
                $saldoFD = $creditoUtulizado + $cantidadQuitar;
                $stmt = $conn->prepare('UPDATE cuentas_credito SET Credito_Utilizado =:saldoFinal WHERE FKCuenta =:fkcuenta');
                $stmt->bindValue(':fkcuenta', $idACuenta, PDO::PARAM_INT);
                $stmt->bindValue(':saldoFinal', $saldoFD);
                if ($stmt->execute() == true) {
                    $uCredito = true;
                } else {
                    $uCredito = false;
                ...
                $stmt = $conn->prepare('INSERT INTO movimientos_cuentas_bancarias_empresa (cuenta_origen_id,Fecha,tipo_movimiento_id,Descripcion,Deposito,Saldo,Comprobado,cuenta_destino_id) VALUES (:fkcuenta,:fecha,:tipo,:descripcion,:deposito,:saldo,:comprobado,:iddestino)');
                $stmt->bindValue(':fkcuenta', $id, PDO::PARAM_INT);
                ...
                $saldoF = $saldoAnterior + $cantidadAgregar;
                $stmt = $conn->prepare('UPDATE cuentas_cheques SET Saldo_Inicial =:saldoFinal WHERE FKCuenta =:fkcuenta');
                $stmt->bindValue(':fkcuenta', $id, PDO::PARAM_INT);
                ...
                $saldoFOtras = $SaldoInicial - $cantidadQuitar;
                ...
                $stmt->bindValue(':tipo', 5);
                $stmt->bindValue(':descripcion', $obs);
                $stmt->bindValue(':retiro', $cantidadQuitar);
                ...
                //UPDATE DE LA CUNETA DESTINO OTRAS
                $saldoFCaja = $SaldoInicialCaja - $cantidadQuitar;
                ...
                //SELECCT DEL SALDO DE CHEQUES DESTINO
                $stmt = $conn->prepare('SELECT * FROM  cuentas_cheques WHERE FKCuenta = :fkcuenta');
                $stmt->bindValue(':fkcuenta', $idACuenta, PDO::PARAM_INT);
                $stmt->execute();
                $rowC = $stmt->fetch();
                ...
                $saldoFCh = $saldoAnteriorD - $cantidadQuitar;
                $stmt = $conn->prepare('UPDATE cuentas_cheques SET Saldo_Inicial = :saldoFinal WHERE FKCuenta =:fkcuenta');
