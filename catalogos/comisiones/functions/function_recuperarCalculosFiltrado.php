...
$fVendedor = $_REQUEST["fVendedor"];
$fFechaDesde = $_REQUEST["fFechaDesde"];
$fFechaHasta = $_REQUEST["fFechaHasta"];
$fEstatus = $_REQUEST["fEstatus"];
...
if($fVendedor!="f") {
    $vendedor=' and com.id_empleado = '.$fVendedor;
} 

if($fFechaDesde!="") {
    if($fFechaHasta==""){
        $fFechaHasta=date("Y-m-d");
    }
    $fechas=' and com.fecha_registro between "'.$fFechaDesde.'" and "'.$fFechaHasta.' 23:59:59"';
} else {
    if($fFechaHasta!=""){
        $between=' and com.fecha_registro <= "'.$fFechaHasta.' 23:59:59"';
    }
}

if($fEstatus!=0) {
    $estado=' and com.estatus = '.$fEstatus;
}
...
$stmt = $conn->prepare('SELECT com.id as idComision, 
                                ...
                                com.porcentaje_comision as porcentaje_comision, 
                                com.saldo_insoluto as saldo_insoluto, 
                                com.estatus as estatus
                        from empleados e 
                            inner join comisiones com on e.PKEmpleado=com.id_empleado 
                            inner join usuarios u on com.id_usuario_registro=u.id
                        where e.empresa_id=:empresa '.$vendedor.''.$fechas.''.$estado);
