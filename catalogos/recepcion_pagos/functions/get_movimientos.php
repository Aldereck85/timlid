...
//si se recibió la factura, se cargan los movimientos de la misma
if (isset($_REQUEST["factura"])) {
    $is_invoice = $_REQUEST["is_invoice"] == 1 ? 2 : 1;

    $factura = 'where tabla.id=' . $_REQUEST["factura"].' and tabla.tipo_CuentaCobrar='.$is_invoice;
...
$stmt = $conn->prepare("SELECT * from (
	...
	inner join (select pp.idpagos, group_concat(' ',fac.serie, fac.folio) as foliosFacturas from facturacion as fac 
				...
				where pp.estatus=1 and pp.empresa_id=:empresa and mov.tipo_CuentaCobrar = 2 and fac.prefactura = 0 group by pp.idpagos) as ff on ff.idpagos=p.idpagos
	...
	inner join cuentas_bancarias_empresa cb on cb.PKCuenta=m.cuenta_destino_id
	inner join usuarios u on m.FKResponsable = u.id
	inner join ventas_directas vd on vd.PKVentaDirecta=m.id_factura
	inner join formas_pago_sat fp on p.forma_pago=fp.id
	...
	inner join (select pp.idpagos, group_concat(' ',vd.Referencia) as foliosFacturas from ventas_directas as vd 
				...
				inner join pagos pp on mov.id_pago = pp.idpagos
				where pp.estatus=1 and pp.empresa_id=:empresa4 and mov.tipo_CuentaCobrar = 1 group by pp.idpagos) as ff on ff.idpagos=p.idpagos
	where p.tipo_movimiento=0 and vd.empresa_id=:empresa5 and vd.empresa_id !=6 and p.estatus=1 and m.tipo_CuentaCobrar = 1 group by p.identificador_pago
    ) as tabla $factura order by fecha_registro desc;");
...
while (($row = $stmt->fetch()) !== false) {
    ...
    /* if ($fn_editar == 1) {
        if ($row['metodo_pago'] == 3) {
            if ($row['estatus'] == 1 || $row['estatus'] == 2) {
                ...
                $acciones = $acciones . '<a class=\"edit-tabs-371\" id=\"editarMP\" href=\"editarPago.php?folio=' . $row['folio'] . '\"><i class=\"fas fa-edit color-primary pointer\" data-toggle=\"tooltip\" data-placement=\"' . $posicion . '\" title=\"Editar\"></i></a> ';
                $acciones = $acciones . '<a class=\"edit-tabs-371\" id=\"facturar\" ><i class=\"fas fa-file-invoice pointer\" data-toggle=\"tooltip\" data-placement=\"' . $posicion . '\" title=\"Facturar\" onclick=\"facturaPago(\'' . $row['folio'] . '\')\"></i></a>';
            }
        } else {
            $acciones = $acciones . '<a class=\"edit-tabs-371\" id=\"editarMP\" href=\"editarPago.php?folio=' . $row['folio'] . '\"><i class=\"fas fa-edit color-primary pointer\" data-toggle=\"tooltip\" data-placement=\"' . $posicion . '\" title=\"Editar\"></i></a>';
        }
    ...
    if ($fn_eliminar == 1) {
        if ($row['estatus'] != 1 && $row['estatus'] != 2) {
            $acciones = $acciones . '<a class=\"edit-tabs-371\" id=\"btnEliminaPago\"><i class=\"fas fa-trash-alt color-primary pointer\" data-toggle=\"tooltip\" data-placement=\"' . $posicion . '\" title=\"Eliminar\" onclick=\"eliminaPago(\'' . $row['folio'] . '\',1)\"></i></a>';
        }
    } */
    ...
    if ($row['estatus'] == 1) {
        ...
        $row['estatus'] = '<span class=\"left-dot red-dot\">En proceso de cancelación</span>';
