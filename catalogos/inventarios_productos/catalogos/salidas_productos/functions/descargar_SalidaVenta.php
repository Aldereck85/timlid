...
$stmt = $conn->prepare("SELECT distinct ispp.folio_salida as folioSalida,
                          ...
                          inner join ventas_directas vd on opps.numero_venta_directa = vd.PKVentaDirecta
                          ...
                          left join facturacion fct on vd.PKVentaDirecta = fct.referencia and fct.tipo = 2 and fct.estatus != 4 and fct.estatus != 5
                          ...
                          inner join sucursales so on opps.sucursal_origen_id = so.id
                          inner join empresas em on so.empresa_id = em.PKEmpresa 
                          left join empleados e on msssi.surtidor_id = e.PKEmpleado
                          inner join ventas_directas vd on opps.numero_venta_directa = vd.PKVentaDirecta
...
//Datos generales de la entrada por orden de compra
$stmtUs = $conn->prepare("SELECT concat(e.Nombres,' ',e.PrimerApellido,' ',e.SegundoApellido) as encargado, us.usuario as email, e.Telefono as telefono
                          from usuarios us
...
$rowUs = $stmtUs->fetch();
  ...
  $GLOBALS["NumeroVenta"] = $row['numeroVenta'];
...
<table>
  ...
  <tr>
    <td class="td3 td4" width="15%">Venta: </td>
    <td class="td3" width="40%">' . $GLOBALS["NumeroVenta"] . '</td>
