<?php
    ...
    for($x = 1;$x<$numPantallas+1;$x++){
      $permis = $_GET['Pantalla'].$x;
      echo $permis;
      /*try{
        $stmt = $conn->prepare('INSERT INTO permisos_pantallas (FKPantalla,FKUsuario,Permiso) VALUES(:fkPantalla,:fkUsuario,:permiso)');
        $stmt->bindValue(':fkPantalla',1);
        $stmt->bindValue(':fkUsuario',$usuarioId);
        $stmt->bindValue(':permiso',$permis);
        $stmt->execute();
      }
      catch(PDOException $ex){
        echo $ex->getMessage();
      }*/ numPantallas=6&Pantalla1=true&Pantalla2=true&Pantalla3=false&Pantalla4=true&Pantalla5=true&Pantalla6=false
    }
    //$pantallaEmpleado =  1;
    //$pantallaEmpleado =  $_GET['numPantalla1'];
    ...
    try{
      $stmt = $conn->prepare('INSERT INTO prueba (Prueba) VALUES(:prueba)');
      $stmt->bindValue(':prueba',$pantallaEmpleado);
      $stmt->execute();
    }
    catch(PDOException $ex){
      echo $ex->getMessage();
