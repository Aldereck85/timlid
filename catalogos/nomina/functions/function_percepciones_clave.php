...
$stmt = $conn->prepare("SELECT tp.id, tp.codigo, tp.concepto, rtp.clave FROM tipo_percepcion as tp LEFT JOIN relacion_tipo_percepcion as rtp ON rtp.tipo_percepcion_id = tp.id  AND rtp.empresa_id = ".$_SESSION['IDEmpresa']);
$stmt->execute();
...
foreach ($percepciones as $p) {
    ...
    $editar = '<span class=\"fas fa-clipboard-list pointer\"  data-toggle=\"modal\" data-target=\"#agregar_clave_tipo\" onclick=\"cargarPercepcion(' . $p['id'] . ');\" data-backdrop=\"static\" data-keyboard=\"false\"></span>';
