...
$stmt = $conn->prepare('call spc_UnidadesMedida()');
$stmt->execute();
$table="";
...
while (($row = $stmt->fetch()) !== false) {
    ...
    $table.='{"id":"<label class=\"textTable\">'.$row['PKUnidadMedida'].'</label>",
        "Unidad":"<label class=\"textTable\">'.$row['UnidadMedida'].'</label><i><img class=\"btnEdit\" data-toggle=\"modal\" data-target=\"#editar_Insumo\" onclick=\"obtenerIdInsumoEditar('.$row['PKUnidadMedida'].');\" src=\"../../../../img/timdesk/edit.svg\"></i>"},';
