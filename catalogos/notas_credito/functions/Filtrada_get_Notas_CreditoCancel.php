...
$table="";
    $all = $conn->prepare("SELECT ncpc.folion_nota,ncpc.id,ncpc.num_serie_nota,ncpc.importe,ncpc.fecha_captura,ncpc.estatus, ncpc.fecha_modifico as f_Cancelacion,ncpc.cliente_id,ncpc.id_Nota_Facturapi,c.NombreComercial, c.PKCliente, ncpc.tipo_nc from notas_cuentas_por_cobrar as ncpc
	inner join clientes as c on c.PKCliente = ncpc.cliente_id where (ncpc.empresa_id = $empresa) and (ncpc.estatus = 2) AND $filtro order by fecha_captura desc;");
    $all->execute();
