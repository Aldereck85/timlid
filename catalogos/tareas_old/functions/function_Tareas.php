...
if($idProyecto > 0){
  $where = " AND tareas.FKProyecto = ".$idProyecto;
}
else{
  $where = "";
}
...
while (($row = $stmt->fetch()) !== false) {
    ...
    if($_SESSION['FKRol'] == 4 || $_SESSION['FKRol'] == 1){
      ...
      $edit = '<a class=\"btn btn-primary\" href=\"#\" data-toggle=\"modal\" data-target=\"#editar_Tarea\" class=\"btn btn-primary\" onclick=\"obtenerIdTareaEditar('.$row['PKTarea'].');\"><i class=\"fas fa-edit\"></i> Editar</a>&nbsp;&nbsp;';
  		$delete ='<a class=\"btn btn-danger\" href=\"#\" data-toggle=\"modal\" data-target=\"#eliminar_Tarea\" class=\"btn btn-danger\" onclick=\"obtenerIdTareaEliminar('.$row['PKTarea'].')\"><i class=\"fas fa-trash-alt\"></i> Eliminar</a>';
      ...
      switch ($estatusID) {
        ...
        $table.='{"Cambiar estatus":"'.$cambiarEstatus.'","Tarea":"'.$row['Tarea'].'","Prioridad":"'.$prioridad .'","Estatus":"'.$estatus.'","Fecha de inicio":"'.$fecha_inicio.'","Fecha de termino":"'.$fecha_termino.'","Responsable":"'.$responsable.'","Asignante":"'.$asignante.'","Nota":"'.$row['Notas'].'","Porcentaje":"'.$avance.'" ,"Acciones":"'.$edit.$delete.'"},';
    }else
    if($row['FKResponsable'] == $_SESSION['PKUsuario']){
      $cambiarEstatus = '<div class=\"dropdown\"><button class=\"btn btn-secondary dropdown-toggle\" type=\"button\" id=\"dropdownMenuButton\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\"> Estatus</button><div class=\"dropdown-menu\" aria-labelledby=\"dropdownMenuButton\"><a class=\"dropdown-item\"  href=\"functions/cambiar_Estatus.php?id='.$row['PKTarea'].'&estatus=1\"><i class=\"fas fa-play\"></i> Por comenzar</a><a class=\"dropdown-item\" href=\"functions/cambiar_Estatus.php?id='.$row['PKTarea'].'&estatus=3\"><i class=\"fas fa-check\"></i> Terminado</a><a class=\"dropdown-item\" href=\"functions/cambiar_Estatus.php?id='.$row['PKTarea'].'&estatus=2\"><i class=\"fas fa-hourglass-half\"></i> En proceso</a><a class=\"dropdown-item\" href=\"functions/cambiar_Estatus.php?id='.$row['PKTarea'].'&estatus=4\"><i class=\"fas fa-times\"></i> No realizado</div></div>';
      ...
      switch ($row['Estatus']) {
        ...
        $table.='{"Cambiar estatus":"'.$cambiarEstatus.'","Tarea":"'.$row['Tarea'].'","Prioridad":"'.$prioridad .'","Estatus":"'.$estatus.'","Fecha de inicio":"'.$fecha_inicio.'","Fecha de termino":"'.$fecha_termino.'","Asignante":"'.$asignante.'","Nota":"'.$row['Notas'].'","Porcentaje":"'.$avance.'","Acciones":"'.$edit.$delete.'"},';
    }
  }
