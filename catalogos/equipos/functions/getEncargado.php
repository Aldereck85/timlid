<?php
    if(isset($_POST['id'])){
        ...
        $stmt1 = $conn->prepare("SELECT i.FKUsuario, i.FKEquipo FROM integrantes_equipo AS i inner JOIN equipos AS e ON e.PKEquipo = i.FKEquipo WHERE PKEquipo = :id");
        $stmt1->execute(array(':id'=>$id));
        $row1 = $stmt1->fetch();
        $idUsuario = $row1['FKUsuario'];
