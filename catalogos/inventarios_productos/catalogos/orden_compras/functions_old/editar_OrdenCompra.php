  ...
  if(isset($_SESSION["Usuario"]) && ($_SESSION["FKRol"] == 4 || $_SESSION["FKRol"] == 1)){
    require_once('../../../include/db-conn.php');
      ...
      if(isset($_POST['txtIdU'])){
        $id =  $_POST['txtIdU'];
        $stmt = $conn->prepare('SELECT * FROM orden_compra WHERE PKOrdenCompra= :id');
                            ...
                            <div class="table-responsive redondear">
                              <table class="table table-hover table-sm">
                                ...
                                <tbody id="lstProductos">
                                  <?php
                                    $stmt = $conn->prepare('SELECT um.PKUnidadMedida,poc.FKProducto,pr.Descripcion,pr.Clave,poc.Cantidad,um.Unidad_de_Medida,um.Piezas_por_Caja,poc.Precio_Unitario FROM productos_oc AS poc
                                    INNER JOIN productos AS pr ON poc.FKProducto = pr.PKProducto
                                    INNER JOIN unidad_medida AS um ON pr.FKUnidadMedida = um.PKUnidadMedida
                                    WHERE poc.FKOrdenCompra = :id');
                                ...
                                <tfoot >
                                  <tr>
                                    ...
                                    <th colspan="2" style="text-align: right;">$ <span id="Subtotal"><?=number_format($importe,2); ?></span></th>
                                    <th>&nbsp</th>
                                  </tr>
                                  <tr>
                                    <th colspan="2"></th>
                                    <th style="text-align: right;">IVA 16%:</th>
                                    <th colspan="2"style="text-align: right;">$ <span id="IVA"><?=number_format($importe *.16,2); ?></span></th>
                                    <th>&nbsp</th>
                                  </tr>
                                  <tr class="total">
                                    <th colspan="2" class="redondearAbajoIzq"></th>
                                    <th style="text-align: right;">Total:</th>
                                    <th colspan="2" style="text-align: right;">$ <span id="Total"><?=number_format($importe*1.16,2); ?></span></th>
  ...
  $('#cmbProducto').change(function(){
    ...
    $.ajax({
      ...
      success:function(r){
        ...
        $('.txtCantidadU').val(data.Cantidad);
      }
    });
  });
  ...
  $(document).ready(function(){
    ...
    $('#btnEditar').click(function(){
      if($('#lstProductos > tr').length < 1){
        ...
        $.ajax({
          type: 'POST',
          url: 'darEdicion_OrdenCompra.php',
