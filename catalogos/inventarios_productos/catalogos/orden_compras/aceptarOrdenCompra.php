...
$stmtLogo = $conn->prepare("SELECT e.logo FROM ordenes_compra oc
                              inner join empresas e on oc.empresa_id = e.PKEmpresa
                            WHERE oc.PKOrdenCompra = (select PKOrdenCompra from ordenes_compra where id_encriptado = :id)");
$stmtLogo->bindValue(':id', $id, PDO::PARAM_INT);
$stmtLogo->execute();
$rowLogo = $stmtLogo->fetch();

$GLOBALS["Logo"] = $rowLogo['logo'];

$stmtEmpresa = $conn->prepare("SELECT e.PKEmpresa FROM ordenes_compra oc
                              inner join empresas e on oc.empresa_id = e.PKEmpresa
                            WHERE oc.PKOrdenCompra = (select PKOrdenCompra from ordenes_compra where id_encriptado = :id)");
$stmtEmpresa->bindValue(':id', $id, PDO::PARAM_INT);
$stmtEmpresa->execute();
$rowEmpresa = $stmtEmpresa->fetch();

$GLOBALS["PKEmpresa"] = $rowEmpresa['PKEmpresa'];


$ruta = $_ENV['RUTA_ARCHIVOS_READ'].$GLOBALS["PKEmpresa"].'/fiscales'.'/';

$GLOBALS["ruta"] = $ruta;
$GLOBALS["logotipo"] = $ruta.$GLOBALS["Logo"];
...
<head>
  ...
  <title>Timlid | Aceptar Orden de compra</title>
  ...
  <script src="../../js/aceptar_ordenCompra.js" charset="utf-8"></script>
...
<body id="page-top">
  ...
  <div id="wrapper">
    ...
    $titulo = 'Aceptar orden de compra';
    $ruta = "../../../";
    ?>
    <!-- End of Sidebar -->
      ...
      <div id="content">
        ...
        <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 sticky-top shadow">
          ...
          <h3 class="d-none d-sm-inline-block mr-auto ml-md-3 my-2 my-md-0 mw-100">
            ...
            <label class="lblTopbar"> <?=$titulo;?></label>
          </h3>
        </nav>
                  ...
                  <div class="row">
                    <div class="col-lg-12">
                      <form action="" method="post" id="frmOrdenCompra">
                        ...
                        <div class="form-group" style="width:70%; margin:auto;">
                          <iframe src="functions/open_OrdenCompra.php?txtId=f899139df5e1059396431415e770c6dd" style="width:100%; height:800px;" frameborder="0" ></iframe>
                        </div>
...
<div class="modal fade" id="rechazar_OrdenCompra" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <form action="" method="POST">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">¿Desea rechazar la orden de compra?</h5>
          <button class="close" type="button" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true" class="text-light">x</span>
          </button>
        </div>
        <input type="hidden" vlaue="" id="estatusIDRechazar" name="estatusIDRechazar">
        <div class="modal-body" style="font-size: 10 px!important; color: red;">Al rechazar la Orden de compra se le notificará al cliente de esta acción vía chat.</div>
        <div class="modal-footer">
          ...
          <button type="submit" onclick="updateEstatusOC($('#estatusIDRechazar').val())" class="btn-custom btn-custom--blue"><span
              class="ajusteProyecto" data-dismiss="modal">Rechazar</span></button>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <form action="" method="POST">
        ...
        <div class="modal-body" style="font-size: 10 px!important; color: red;">Al aceptar la Orden de compra se le notificará al cliente de esta acción vía chat.</div>
        <div class="modal-footer">
          <button class="btn-custom btn-custom--border-blue btnCancelarActualizacion" type="button"
            data-dismiss="modal"><span class="ajusteProyecto">Cancelar</span></button>
          <button type="submit" onclick="updateEstatusOC($('#estatusIDAceptar').val())" class="btn-custom btn-custom--blue"><span
              class="ajusteProyecto" data-dismiss="modal">Aceptar</span></button>
        </div>
      </form>
    </div>
  </div>
