...
$pdf->SetTitle('Recibo liquidación');
$pdf->SetSubject('Recibo liquidación');
$pdf->SetKeywords('Recibo, liquidación');
...
$stmt = $conn->prepare("SELECT e.PKEmpleado, e.Primer_Nombre, e.Segundo_Nombre, e.Apellido_Paterno, e.Apellido_Materno, de.Fecha_Ingreso, 
                               p.Sueldo_semanal_bruto, p.Puesto, de.Deuda_Restante,t.Horas_de_trabajo, f.*, l.*
                            FROM empleados AS e
...
$html = '<p style="text-align: center;font-weight:bold;">LIQUIDACIÓN</p>'; 
$pdf->writeHTML($html, true, false, true, false, '');
...
$html = '<table> 
      <tr>
        <td colspan="3"></td>
			...
			<tr>
				...
				<td align="center">'.number_format($Salario3MesesBD,2).'</td>
				<td></td>
			</tr>
			<tr>
				<td>20 DÍAS POR CADA AÑO DE SERVICIO</td>
				<td align="center">'.number_format($DiasAnioServicioBD,2).'</td>
				<td></td>
			</tr>
			<tr>
				<td>PRIMA ANTIGÜEDAD</td>
				<td align="center">'.number_format($PrimaAntiguedadBD,2).'</td>
				<td></td>
			</tr>
      <tr>
        <td colspan="3"></td>
			...
			<tr style="font-weight:bold">
				<td>TOTAL DE INGRESOS POR INDEMNIZACION</td>
				<td align="center">'.number_format($TotalIndemnizacionBD,2).'</td>
				<td>'.number_format($TotalIngresosIndemnizacionBD,2).'</td>
			</tr>
      <tr>
        <td colspan="3"></td>
      </tr>
      <tr>
        <td>EXENCIÓN</td>
        <td align="center">'.number_format($ExencionBD,2).'</td>
        <td></td>
      </tr>
      <tr>
        <td>BASE GRAVABLE</td>
        <td align="center">'.number_format($base_gravable_liquidacionBD,2).'</td>
        <td></td>
      </tr>
      <tr>
        <td>DETERMINACIÓN DEL ISR ART. 113 LISR</td>
        <td align="center">'.number_format($ISR113LISRBD,2).'</td>
        <td></td>
      </tr>
      <tr>
        <td>ISR DE LA INDEMNIZACIÓN</td>
        <td align="center">'.number_format($ISRIndemnizacionBD,2).'</td>
        <td></td>
      </tr>
      <tr>
        <td>ISR ART. 142 RLISR</td>
        <td align="center">'.number_format($ISR142RLISRBD,2).'</td>
        <td></td>
      </tr>
      <tr>
        <td>TOTAL DEDUCCIONES</td>
        <td align="center"></td>
        <td>'.number_format($TotalDeduccionesBD,2).'</td>
      </tr>
			<tr>
				<td colspan="3"></td>
			</tr>
			<tr style="font-weight:bold">
				...
				<td></td>
				<td align="center">'.number_format($TotalLiquidacionBD,2).'</td>
			</tr>
		 </table>';
$pdf->writeHTML($html, true, false, true, false, '');
...
$html = '<br><br><p style="text-align: justify;">Por tal motivo extiendo el más amplio y valedero RECIBO LIQUIDACIÓN que en derecho proceda, haciendo constar que no se me adeuda prestación alguna de ninguna naturaleza, firmando el presente para constancia.</p>'; 
$pdf->writeHTML($html, true, false, true, false, '');
